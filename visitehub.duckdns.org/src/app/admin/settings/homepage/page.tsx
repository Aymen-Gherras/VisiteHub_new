          ...
          {featured.map((row, idx) => (
            <div key={(row.property as any).id} className="flex items-center gap-4 border rounded-lg p-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-500">{row.property.wilaya}{row.property.daira ? `, ${row.property.daira}` : ''} • {typeof row.property.price === 'number' ? row.property.price.toLocaleString('fr-FR') : row.property.price || 'N/A'}</div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => {
                  setFeatured(prev => {
