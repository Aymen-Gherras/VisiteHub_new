...
import { Agence, CreateAgenceDto, apiService } from '../../../../../api';

interface ImagesSectionProps {
  agenceData: Partial<Agence>;
  setAgenceData: (data: Partial<Agence>) => void;
  onPrevious: () => void;
  onAgenceCreated: (agence: Agence) => void;
...
export default function ImagesSection({
  ...
  const [logoPreview, setLogoPreview] = useState<string>(agenceData.logo || '');
  const [coverImagePreview, setCoverImagePreview] = useState<string>(agenceData.coverImage || '');
  ...
  const uploadToCloudinary = async (file: File, type: 'logo' | 'cover'): Promise<string> => {
    ...
    formData.append('folder', `agences/${type}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let logoUrl = agenceData.logo;
      let coverImageUrl = agenceData.coverImage;
      ...
      let result: Agence;
      if (isEditing && agenceId) {
        result = await apiService.updateAgence(agenceId, finalData, token);
      } else {
        // Ensure required fields are present for creation
        if (!finalData.name) {
          throw new Error('Agency name is required');
        }
        result = await apiService.createAgence(finalData as CreateAgenceDto, token);
      }

      onAgenceCreated(result);
    } catch (error) {
      console.error('Error saving agence:', error);
      alert(error instanceof Error ? error.message : 'Failed to save agence');
    } finally {
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600">Upload agency logo and cover image for professional presentation</p>
      </div>
          ...
          <div className="w-full h-48 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50">
            {coverImagePreview ? (
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-400">Optional banner image for agency profile</p>
              </div>
            )}
          </div>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-purple-700">
              ...
              The logo will appear in property listings and the cover image will be displayed on the agency profile page.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between pt-6">
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <span>{isEditing ? 'Update Agence' : 'Create Agence'}</span>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
