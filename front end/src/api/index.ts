...
export class ApiService {
  ...
  private async makeRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    try {
      ...
      if (response.status === 204) {
        console.log('API Service: Success response (204 No Content)');
        return {} as T;
      }
      ...
      if (!text || text.trim() === '') {
        console.log('API Service: Success response (empty body)');
        return {} as T;
      }
      ...
      try {
        ...
        console.log('API Service: Success response (non-JSON, treating as success)');
        return {} as T;
      }
    } catch (error) {
  ...
  async updateProperty(id: string, data: Partial<CreatePropertyDto>, token?: string): Promise<Property> {
    ...
    if (payload.price !== undefined) {
      ...
      console.log('🔍 API updateProperty - Price in payload:', payload.price, 'Type:', typeof payload.price);
    }
    
    const jsonBody = JSON.stringify(payload);
    console.log('🔍 API updateProperty - JSON body:', jsonBody);
    console.log('🔍 API updateProperty - Price in JSON:', JSON.parse(jsonBody).price, 'Type:', typeof JSON.parse(jsonBody).price);
