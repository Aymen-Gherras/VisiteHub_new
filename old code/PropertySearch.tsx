...
const PropertySearch = () => {
  ...
  const filteredProperties = properties.filter(property => {
    ...
    const matchesTransaction = filters.transactionType === 'tous' || property.transactionType === filters.transactionType;
    const matchesWilaya = !filters.selectedWilaya || property.wilaya.toLowerCase().includes(filters.selectedWilaya.toLowerCase());
    const matchesDaira = !filters.selectedDaira || (property as any).daira?.toLowerCase().includes(filters.selectedDaira.toLowerCase());
    const matchesType = !filters.selectedPropertyType || property.type === filters.selectedPropertyType;
    ...
    <section id="biens" className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mt-12">
          <Link href="/properties" className="bg-slate-800 text-white px-8 py-3 rounded-xl hover:bg-slate-700 transition-colors inline-block">
            Voir plus de biens
          </Link>
        </div>
      </div>
